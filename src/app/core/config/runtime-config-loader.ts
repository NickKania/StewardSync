 ...
 * 2. HTML meta tags
 * 3. Browser environment (if available)
 */
...
export async function loadRuntimeConfig(): Promise<RuntimeConfig> {
  ...
  config = {
    convexUrl: (typeof process !== 'undefined' && process.env?.PUBLIC_CONVEX_URL) || DEFAULT_CONFIG.convexUrl,
    enableDevLogin: coerceBoolean((typeof process !== 'undefined' && process.env?.PUBLIC_ENABLE_DEV_LOGIN) || 'false'),
    discordClientId: (typeof process !== 'undefined' && process.env?.DISCORD_CLIENT_ID) || DEFAULT_CONFIG.discordClientId,
    discordClientSecret: (typeof process !== 'undefined' && process.env?.DISCORD_CLIENT_SECRET) || DEFAULT_CONFIG.discordClientSecret
  };
