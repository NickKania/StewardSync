              ...
              <app-card title="Your Review">
                <div class="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      [class.input-error]="form.get('reviewNotes')?.invalid && form.get('reviewNotes')?.touched"
                      placeholder="Your assessment of the incident, findings, and observations..."
                      rows="5"
                    ></textarea>
                    @if (form.get('reviewNotes')?.invalid && form.get('reviewNotes')?.touched) {
                      <p class="mt-1 text-sm text-red-600">Review notes are required</p>
                    }
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p class="text-xs text-gray-500 mt-1">Optional: Reference to video evidence</p>
                  </div>
                </div>
                ...
                <div card-footer class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row justify-between gap-3">
                  ...
                  <div class="flex gap-3">
                    ...
                    @if (authService.hasRole('head_steward', 'event_manager')) {
                      <app-button
                        type="button"
                        variant="success"
                        [loading]="submitting()"
            ...
            <app-card title="Incident Details">
              <dl class="space-y-4">
                ...
                <div>
                  ...
                  <dd class="text-gray-900">{{ report()?.reportDate | dateFormat:'PPp' }}</dd>
                </div>
              </dl>
            </app-card>
...
export class ReviewFormComponent implements OnInit, OnDestroy {
  ...
  async onSubmit(markAsReviewed = false): Promise<void> {
    ...
    try {
      ...
      if (markAsReviewed) {
        await this.convex.mutation(
          this.convex.api.reports.markAsReviewed,
          { reportId: this.reportId as any }
        );
